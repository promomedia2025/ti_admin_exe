...
// Function to get the full app config from userData
function getAppConfig() {
  try {
    ...
    console.warn("⚠️ Failed to read app config:", error.message);
  }
  return {};
}
...
function getSavedNotificationOutputDevice() {
  const config = getAppConfig();
  const id = config.notificationOutputDeviceId;
  return typeof id === "string" && id.length > 0 ? id : null;
}

function saveNotificationOutputDevice(deviceId) {
  try {
    const userDataPath = app.getPath("userData");
    const configPath = path.join(userDataPath, "app-config.json");
    const config = getAppConfig();
    config.notificationOutputDeviceId =
      deviceId == null || deviceId === "" ? undefined : deviceId;
    fs.writeFileSync(configPath, JSON.stringify(config, null, 2), "utf8");
    return true;
  } catch (error) {
    console.warn("⚠️ Failed to save notification output device:", error.message);
    return false;
  }
}

// Function to save domain to userData (merges with existing config)
...
function getConfiguredURL() {
  const defaultDomain = "localhost:3001";
  const protocol = "http://";
  const urlPath = "/admin";
...
function createWindow() {
  ...
  if (!app.isPackaged) {
    ...
    mainWindow.webContents.once("did-finish-load", () => {
      ...
      // Notify API that the app is now active (status 2)
      // Delay slightly to give renderer time to set toggle config
      setTimeout(() => {
        sendToggleSchedulesRequest(2);
      }, 1000);
    });
  ...
  mainWindow.webContents.on("before-input-event", (event, input) => {
    if (
      input.key === "F1" ||
      (input.control && input.key.toLowerCase() === "i")
    ) {
      event.preventDefault();
      showAboutDialog();
    }
...
autoUpdater.on("update-available", (info) => {
  ...
  if (mainWindow) {
    mainWindow.setTitle(
      `Promo Media v${appVersion} - Update available, downloading...`,
    );
  }
...
autoUpdater.on("update-downloaded", (info) => {
  ...
  if (mainWindow) {
    mainWindow.setTitle(
      `Promo Media v${appVersion} - Update ready, will install on restart`,
    );
  }
  ...
  console.log(
    "✅ Update ready. Will install automatically when application closes.",
  );
...
ipcMain.handle("check-for-updates", async () => {
  try {
    ...
    if (
      ...
      console.warn(
        "⚠️ Update check failed due to temp file issue (non-critical)",
      );
      return {
        success: false,
        error: "Temp file access issue - updates may not be available",
      };
    }
...
ipcMain.handle("download-update", async () => {
  try {
    ...
    if (
      ...
      return {
        success: false,
        error: "Temp file access issue - cannot download update",
      };
    }
...
// Native notification sound: loop until stop-notification-sound is invoked
let notificationStopRequested = false;
let notificationLoopPromise = null;
let currentNotificationProcess = null; // so we can kill it for instant stop

function resolveNotificationSoundPath(src) {
  let filePath =
  ...
  const tempPath = path.join(tempDir, `promo-media-${baseName}`);
  try {
...
// Play one time; on Windows use spawn so we can kill the process for instant stop
function playNotificationFileOnce(pathToPlay) {
  if (process.platform === "win32") {
    const escapedPath = pathToPlay.replace(/'/g, "''");
    const script =
      `Add-Type -AssemblyName presentationCore; $p = New-Object system.windows.media.mediaplayer; $p.Open('${escapedPath}'); $p.Volume = 1; $p.Play(); Start-Sleep -Milliseconds 500; while ($p.Position -lt $p.NaturalDuration.TimeSpan -and $p.Source) { Start-Sleep -Milliseconds 200 }; Exit`;
    return new Promise((resolve, reject) => {
      const child = spawn("powershell", ["-NoProfile", "-Command", script], {
        stdio: "ignore",
        windowsHide: true,
      });
      currentNotificationProcess = child;
      child.on("error", (err) => {
        currentNotificationProcess = null;
        reject(err);
      });
  ...
  return sound.play(pathToPlay);
}

ipcMain.handle("play-notification-sound", async (event, src) => {
  try {
    const deviceId = getSavedNotificationOutputDevice();
    console.log("[notification-sound] play requested, saved deviceId:", deviceId ?? "(none)", "mainWindow:", !!mainWindow, "destroyed:", mainWindow?.isDestroyed());

    // If user chose a specific output device, play from renderer (so setSinkId is used)
    if (deviceId && mainWindow && !mainWindow.isDestroyed()) {
      const fileName =
        src && typeof src === "string" && src.trim().length > 0
          ? src.trim()
          : "phone-ringtone-normal-444775.mp3";
      console.log("[notification-sound] sending to renderer with deviceId:", deviceId, "src:", `app://${fileName}`);
      mainWindow.webContents.send("play-notification-with-device", {
        deviceId,
        src: `app://${fileName}`,
      });
      return { success: true };
    ...
    if (!deviceId) console.log("[notification-sound] no device saved, using main process (default output)");
    if (!mainWindow?.isDestroyed?.()) console.log("[notification-sound] mainWindow missing or destroyed, using main process");

    notificationStopRequested = true;
    if (notificationLoopPromise) await notificationLoopPromise;

    const pathToPlay = resolveNotificationSoundPath(src);
    notificationStopRequested = false;

    notificationLoopPromise = (async () => {
      while (!notificationStopRequested) {
        await playNotificationFileOnce(pathToPlay);
        if (notificationStopRequested) break;
      }
      notificationLoopPromise = null;
    })();
...
ipcMain.handle("stop-notification-sound", async () => {
  notificationStopRequested = true;
  if (currentNotificationProcess) {
    currentNotificationProcess.kill();
    currentNotificationProcess = null;
  }
  if (notificationLoopPromise) await notificationLoopPromise;
  if (mainWindow && !mainWindow.isDestroyed()) {
    mainWindow.webContents.send("stop-notification");
  }
  return { success: true };
});

ipcMain.handle("set-notification-output-device", async (event, deviceId) => {
  console.log("[notification-sound] set output device:", typeof deviceId === "string" ? deviceId || "(default)" : deviceId);
  return { success: saveNotificationOutputDevice(deviceId) };
});

ipcMain.handle("get-notification-output-device", async () => {
  return getSavedNotificationOutputDevice();
});

// Toggle schedules config - set from renderer (Next.js)
ipcMain.handle("set-toggle-config", async (event, { url, locationId }) => {
  toggleConfig = {
    url: url || toggleConfig.url || TOGGLE_SCHEDULES_URL,
    locationId:
      typeof locationId === "number" ? locationId : toggleConfig.locationId,
  };
  return { success: true, config: toggleConfig };
});
...
// Toggle-all-schedules API: POST on app open (status 2) and on app close (status 0)
// Default URL based on saved domain (used as a fallback if renderer doesn't configure it)
const TOGGLE_SCHEDULES_URL = `https://${getSavedDomain()}/api/admin/toggle-schedule`;

// Dynamic config that can be set from the renderer (Next.js) via IPC
let toggleConfig = {
  url: null, // full URL like https://.../toggle-all-schedules
  locationId: null,
};

function sendToggleSchedulesRequest(status, url, locationId, onDone) {
  const targetUrl = url || toggleConfig.url || TOGGLE_SCHEDULES_URL;
  const targetLocationId = locationId ?? toggleConfig.locationId ?? 1;

  if (!targetUrl) {
    console.warn("⚠️ No toggle schedules URL configured, skipping request");
    if (onDone) onDone();
    return;
  }

  const body = JSON.stringify({ status, location_id: targetLocationId });
...
let isClosingWithRequest = false;
app.on("before-quit", (event) => {
  if (isClosingWithRequest) return;
  event.preventDefault();
  isClosingWithRequest = true;
  // Notify API that the app is closing (status 0)
  sendToggleSchedulesRequest(0, null, null, () => app.quit());
});
...
async function handlePrint(options) {
  ...
  try {
    console.log(
      "🖨️ Print request received - Full options:",
      JSON.stringify(options, null, 2),
    );
    console.log(
      "🖨️ Extracted deviceName:",
      deviceName,
      "(type:",
      typeof deviceName,
      ")",
    ...
    if (mediaBoxMatch) {
      ...
      if (
        paperSize &&
        (paperSize.toLowerCase().includes("80mm") ||
          paperSize.toLowerCase().includes("80"))
      ) {
        if (Math.abs(widthMm - 80) >= 5) {
          console.warn(
            "⚠️ WARNING: PDF width is NOT 80mm! PDF appears to be wrong size.",
          );
          console.warn(
            "⚠️ This will cause printing issues. PDF should be 80mm (226.77 points) wide.",
          );
        }
      }
    } else {
    ...
    const isPlaceholder =
      trimmedDeviceName.toLowerCase() === "default printer" ||
    ...
    if (paperSize) {
      ...
      if (paperSizeLower.includes("80mm") || paperSizeLower.includes("80")) {
        ...
        console.log(
          "📄 Thermal printer configured: 80mm paper, no scaling, no margins",
        );
    ...
    try {
      ...
      console.log(
        "✅ PDF printed successfully to:",
        printOptions.printer || "default printer",
      );
      ...
      setTimeout(() => {
        try {
          if (fs.existsSync(tempFilePath)) {
            ...
            console.log(
              "🗑️ Temp file deleted after print completion:",
              tempFilePath,
            );
          }
        } catch (err) {
...
ipcMain.handle("get-printers", async (event) => {
  try {
    ...
    if (process.platform === "win32") {
      ...
      try {
        ...
        const { stdout: defaultStdout } = await execAsync(
          'powershell -Command "$default = Get-Printer | Where-Object {$_.Default -eq $true}; if ($default) { $default.Name } else { \\\"\\\" }"',
        );
    ...
    printers.forEach((printer, index) => {
      console.log(`\nPrinter ${index + 1}:`, {
        ...
        printerState:
          ...
          "N/A (PowerShell only provides PrinterStatus)",
    ...
    const activePrinters = printers.filter((printer) => {
      ...
      if (
        printer.options &&
        printer.options["printer-is-accepting-jobs"] === false
      ) {
        shouldInclude = false;
      ...
      if (
        ...
        if (typeof state === "string") {
          ...
          if (
            lowerState.includes("offline") ||
            lowerState.includes("error") ||
            lowerState.includes("stopped") ||
            lowerState.includes("paused")
          ) {
            shouldInclude = false;
      ...
      if (
        shouldInclude &&
        printer.printerState !== undefined &&
      ...
      if (!shouldInclude) {
        console.log(
          `❌ Filtered out printer "${printer.name}": ${filterReason}`,
        );
      } else {
    ...
    console.log(
      "📤 Returning printer list:",
      JSON.stringify(printerList, null, 2),
    );
